...
data_dir = 'E:/NLCS/data/test'

# Tạo các thư mục để lưu những hình ảnh được chia
os.makedirs('E:/NLCS/data/valid/images', exist_ok=True)
os.makedirs('E:/NLCS/data/valid/labels', exist_ok=True)
...
test_files = np.random.choice(files, size=int(len(files) * 0.3), replace=False)

# Di chuyển các tệp đã chọn sang thư mục kiểm thử
for file in test_files:
    # Di chuyển tệp hình ảnh, 
    shutil.move(os.path.join(data_dir, 'images', file), 'E:/NLCS/data/valid/images')
    ...
    # Di chuyển tệp nhãn
    shutil.move(os.path.join(data_dir, 'labels', label_file), 'E:/NLCS/data/valid/labels')
